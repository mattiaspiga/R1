...
knn_pred = knn(train_knn, test_knn, mydf$Species[index_train], k = 1)
...
#GRAFICO30BIS
dt_test_knn = as.data.frame(test_knn)
colnames(dt_test_knn) = c("Sepal.Length","Sepal.Width","Petal.Length","Petal.Width")

# Create a dataframe to simplify charting
plot.df = data.frame(dt_test_knn, predicted = knn_pred)
...
plot.df1 = data.frame(x = plot.df$Petal.Length,  y = plot.df$Sepal.Width, predicted = plot.df$predicted)

find_hull = function(df) df[chull(df$x, df$y), ]
boundary = ddply(plot.df1, .variables = "predicted", .fun = find_hull)

ggplot(plot.df, aes(Petal.Length, Sepal.Width, color = predicted, fill = predicted)) + 
  geom_point(size = 5) + 
  geom_polygon(data = boundary, aes(x,y), alpha = 0.3)
